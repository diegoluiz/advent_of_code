...
fn part_01(scanners: &Vec<Scanner>) -> String {
...
fn part_02(scanners: &Vec<Scanner>) -> String {
    let mut scanners = scanners.clone();
    ...
    for i in 0..locations.len() {
        for j in i..locations.len() {
            let dist = (locations[i].0 - locations[j].0).abs() + (locations[i].1 - locations[j].1).abs() + (locations[i].2 - locations[j].2).abs();
...
fn main() {
    ...
    let part_01_output = part_01(&scanners);
    println!(
    ...
    let part_02_output = part_02(&scanners);
    println!(
